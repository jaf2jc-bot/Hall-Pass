...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [currentPeriod, setCurrentPeriod] = useState<string>('Period 3');
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      // Middle school schedule approx calculation
      if (totalMins < 460) setCurrentPeriod('Before School');
      else if (totalMins < 515) setCurrentPeriod('Period 1 (8:00 - 8:45)');
      else if (totalMins < 570) setCurrentPeriod('Period 2 (8:50 - 9:35)');
      else if (totalMins < 625) setCurrentPeriod('Period 3 (9:40 - 10:25)');
      else if (totalMins < 680) setCurrentPeriod('Period 4 (10:30 - 11:15)');
      else if (totalMins < 735) setCurrentPeriod('Period 5 / Lunch (11:20 - 12:05)');
      else if (totalMins < 790) setCurrentPeriod('Period 6 (12:10 - 12:55)');
      else if (totalMins < 845) setCurrentPeriod('Period 7 (1:00 - 1:45)');
      else if (totalMins < 900) setCurrentPeriod('Period 8 (1:50 - 2:35)');
      else setCurrentPeriod('After School');
    };
  ...
  // RBAC Navigation items based on role
  const allNavItems = [
    { id: 'student', label: 'My Hall Pass', icon: GraduationCap, roles: ['student', 'teacher', 'admin'] },
    { 
      id: 'currently-out', 
      label: 'Live Hallway Monitor', 
      icon: Eye, 
      roles: ['teacher', 'admin'],
      badge: activePassCount > 0 ? activePassCount : undefined 
    },
    { id: 'teacher', label: 'Teacher Desk', icon: UserCheck, roles: ['teacher', 'admin'] },
    { id: 'admin', label: 'Admin Security & Rosters', icon: Shield, roles: ['admin'] },
    { id: 'history', label: 'Pass History', icon: History, roles: ['teacher', 'admin'] }
  ];
    ...
    <header className="sticky top-0 z-40 bg-purple-950 text-white shadow-lg border-b-4 border-amber-400">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 flex flex-wrap items-center justify-between gap-3 border-b border-purple-900/60">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-purple-300 font-semibold hidden sm:inline flex items-center gap-1">
                <CheckCircle className="w-3 h-3 text-emerald-400" />
                @{ALLOWED_DOMAIN}
              </span>
            </div>
            <h1 className="text-lg sm:text-xl font-bold tracking-tight text-white leading-tight flex items-center gap-1.5">
              Jackson Memorial <span className="text-amber-400 font-extrabold">e-Hall Pass</span>
            </h1>
          </div>
        </div>

        {/* Live Clock & Period Info */}
        <div className="hidden md:flex items-center gap-3 bg-purple-900/70 border border-purple-800/80 px-3.5 py-1.5 rounded-lg text-xs">
          ...
          <span className="text-purple-200 font-medium">{currentPeriod}</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {soundEnabled ? <Volume2 className="w-4 h-4 text-amber-400" /> : <VolumeX className="w-4 h-4 text-purple-400" />}
          </button>

          {/* Google Workspace Sign-In / Account Dropdown */}
                ...
                <div className="text-left hidden sm:block">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-[10px] font-black uppercase px-1.5 py-0.2 rounded ${
                      ...
                      currentRole === 'teacher' ? 'bg-amber-400 text-purple-950' : 'bg-emerald-500 text-white'
                    }`}>
                      {currentRole}
                    </span>
                  </div>
                ...
                >
                  <div className="flex items-center gap-3 pb-3 border-b border-slate-100">
                    ...
                    <div className="overflow-hidden">
                      <p className="font-bold text-sm text-slate-900 truncate">{currentUser?.displayName}</p>
                  ...
                  <div className="py-2.5">
                    <p className="text-[11px] text-slate-500">
                      Authenticated via Google Workspace SSO. Access permissions are securely scoped to your middle-school role.
                    </p>
                  </div>
                  ...
                  >
                    ...
                    <span>Sign Out from JMMS e-Pass</span>
                  </button>
                </div>
              )}
            </div>
          ) : (
            /* Google Workspace SSO Sign-In Button */
            <div className="flex items-center gap-2">
              <button
              ...
              <div className="relative">
                <button
                  ...
                  type="button"
                  onClick={() => setShowUserSwitcher(!showUserSwitcher)}
                  className="flex items-center gap-1.5 bg-purple-900 hover:bg-purple-800 text-purple-200 px-2.5 py-1.5 rounded-lg border border-purple-700 text-xs transition"
                  title="Switch testing preview role"
                >
                ...
                {showUserSwitcher && (
                  <div 
                    className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-2xl border border-purple-200 text-slate-800 z-50 p-3 max-h-[80vh] overflow-y-auto"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-100">
                      <span className="text-xs font-bold text-purple-950 uppercase tracking-wide">Test / Demo Switcher</span>
                      <button onClick={() => setShowUserSwitcher(false)} className="text-slate-400 hover:text-slate-600 text-xs">Close</button>
                    </div>

                    <div className="grid grid-cols-3 gap-1 bg-slate-100 p-1 rounded-lg mb-3">
                      <button
                        onClick={() => {
                          setRole('student');
                        ...
                        className={`py-1 text-xs font-semibold rounded-md transition ${
                          currentRole === 'student' ? 'bg-purple-950 text-amber-300 shadow-sm' : 'text-slate-600'
                        }`}
                      >
                        Student
                      </button>
                      <button
                        ...
                        className={`py-1 text-xs font-semibold rounded-md transition ${
                          currentRole === 'teacher' ? 'bg-purple-950 text-amber-300 shadow-sm' : 'text-slate-600'
                        }`}
                      >
                        Teacher
                      </button>
                      <button
                        ...
                        className={`py-1 text-xs font-semibold rounded-md transition ${
                          currentRole === 'admin' ? 'bg-purple-950 text-amber-300 shadow-sm' : 'text-slate-600'
                        }`}
                      >
                        Admin
                      </button>
                    ...
                    {currentRole === 'student' && (
                      <div className="space-y-1">
                        <p className="text-[11px] text-slate-500 font-medium">Select Student Persona:</p>
                          ...
                          {students.map((student) => (
                            <button
                              ...
                              className={`w-full text-left px-2 py-1 rounded text-xs flex items-center justify-between ${
                                activeStudent?.id === student.id ? 'bg-purple-100 text-purple-950 font-bold' : 'hover:bg-slate-50 text-slate-700'
                              }`}
                            >
                              <span>{student.firstName} {student.lastName}</span>
                              <span className="text-[10px] text-slate-400">ID: #{student.studentId}</span>
                            </button>
                          ))}
                        </div>
                      </div>
      ...
      {/* Auth Error Banner if domain validation fails */}
      {authError && (
        ...
        <nav className="flex space-x-1 sm:space-x-2 overflow-x-auto py-1 scrollbar-none">
          {visibleNavItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 ${isActive ? 'text-amber-400' : 'text-purple-300'}`} />
